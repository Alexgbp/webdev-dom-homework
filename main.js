...
// Функция котрая пытается определить есть ли в переменной User данные из LocalStorage.Если да то это True и значит происходит одна логика(Отрисовывается комменты и форма) если false другая логика(отрисовыается комменты и строка "Пройдите автторизацию")
...
export function getFetchPromise() {
  getComments() // вызываем функцию посылающую get запрос на сервер для получения списка комментов 
    .then((dataResponse) => {
      const newList = dataResponse.comments.map((element) => { // тут мы в переменную newList ложим наш обьект который пришел в 
